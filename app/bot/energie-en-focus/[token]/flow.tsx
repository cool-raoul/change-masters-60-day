// File: app/bot/energie-en-focus/[token]/flow.tsx
//
// Client-flow voor Energie & Focus score-bot.
// Stappen: intro -> intekenen -> vragen -> uitkomst+advies -> opt-in-klaar
...
export function EnergieEnFocusFlow({
  token,
  memberId,
  memberVoornaam,
  bestellinks,
  ...
  const [antwoorden, setAntwoorden] = useState<Partial<EFAntwoorden>>({});
    ...
    <div className="mx-auto max-w-2xl px-4 py-6 sm:py-10 space-y-6">
      ...
      <div className="rounded-3xl bg-white/85 backdrop-blur-md border border-white shadow-lg p-6 sm:p-8">
        ...
        {stap === "uitkomst" && inteken && (
          <Uitkomst
            token={token}
            antwoorden={antwoorden as EFAntwoorden}
            inteken={inteken}
            memberVoornaam={memberVoornaam}
...
function Header({
  ...
  const totaal = 5;
  const pct = (stapNummer[stap] / totaal) * 100;

  return (
    <header className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-amber-200 via-orange-100 to-sky-50 px-6 py-7 sm:px-8 sm:py-8 shadow-sm">
      <div
        aria-hidden
      ...
      <div className="relative">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-orange-700 text-xs font-semibold uppercase tracking-widest">
              Energie & Focus
            </div>
  ...
  return (
    <div className="text-center">
      <div className="relative mx-auto mb-2 h-24 w-24">
        <div className="absolute inset-0 rounded-full bg-gradient-to-br from-amber-200 via-orange-200 to-sky-100 blur-xl opacity-70" />
        <div className="relative flex h-full w-full items-center justify-center rounded-full bg-gradient-to-br from-amber-100 to-orange-50 text-5xl shadow-md ring-4 ring-white/60">
          ⚡
        </div>
      </div>

      <div className="mt-4 text-orange-600 text-sm font-semibold uppercase tracking-widest">
        Energie & Focus
      </div>
      ...
      {/* Internet-perspectief direct bij intro */}
      ...
      <div className="mt-6 flex flex-wrap items-center justify-center gap-2 text-xs">
        <span className="rounded-full bg-white/80 px-3 py-1.5 text-orange-700 font-medium shadow-sm border border-orange-100">
          ⏱ 5 minuten
        </span>
        <span className="rounded-full bg-white/80 px-3 py-1.5 text-orange-700 font-medium shadow-sm border border-orange-100">
          📊 Persoonlijke score
      ...
      <button
        type="button"
        onClick={onStart}
        className="mt-8 group rounded-full bg-gradient-to-r from-orange-600 to-amber-600 px-10 py-4 text-white text-base font-semibold shadow-lg hover:shadow-xl hover:from-orange-700 hover:to-amber-700 transition-all"
      >
        Ja, ik start
      ...
      <p className="mt-6 text-xs text-gray-500">
        Geen medisch advies. Wel echte uitleg over hoe je lichaam werkt.
      </p>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="text-center">
        ...
        <div className="mt-3 text-orange-600 text-xs font-semibold uppercase tracking-widest">
          Voor je begint
        </div>
      ...
      <div className="mt-7 flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-full bg-gradient-to-r from-orange-600 to-amber-600 px-6 py-3 text-white text-base font-semibold shadow-md hover:shadow-lg disabled:from-gray-300 disabled:to-gray-300 disabled:shadow-none transition"
        >
...
function Vragen({
  ...
  antwoorden: Partial<EFAntwoorden>;
  onUpdate: (sleutel: keyof EFAntwoorden, waarde: EFAntwoord) => void;
  onKlaar: () => void;
}) {
  const [index, setIndex] = useState(0);
  const huidige = EF_VRAGEN[index];
  const huidigeKlaar = antwoorden[huidige.sleutel] !== undefined;

  function volgende() {
    if (!huidigeKlaar) return;
    if (index === EF_VRAGEN.length - 1) {
      onKlaar();
      return;
    }
    setIndex((i) => i + 1);
  ...
  const pct = ((index + 1) / EF_VRAGEN.length) * 100;

  return (
    <div>
      <div>
        <div className="flex justify-between text-xs text-orange-700 font-medium mb-1.5">
          <span>
            Vraag {index + 1} van {EF_VRAGEN.length}
          </span>
            ...
            return (
              <button
                ...
                className={`group w-full text-left rounded-2xl px-4 py-3.5 border-2 transition-all flex items-center gap-3 ${
                  actief
                    ? "bg-gradient-to-r from-orange-50 to-amber-50 border-orange-400 text-orange-900 shadow-md scale-[1.01]"
                    : "bg-white/80 border-gray-200 text-gray-700 hover:border-orange-300 hover:bg-orange-50/40"
                }`}
              >
                <span
      ...
      <div className="mt-8 flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-full bg-gradient-to-r from-orange-600 to-amber-600 px-6 py-2.5 text-white text-sm font-semibold shadow-md hover:shadow-lg disabled:from-gray-300 disabled:to-gray-300 disabled:shadow-none transition"
        >
          {index === EF_VRAGEN.length - 1 ? "Toon mijn score ✨" : "Volgende →"}
        </button>
      </div>
    </div>
  );
...
// ============================================================
// Uitkomst: score + advies-blokken + product-brug
// ============================================================
...
function Uitkomst({
  ...
  // Welke thema-blokken tonen we uitgebreid? De top-thema's eerst,
  // overige thema's daarna kort.
  const topThemaBlokken = uitkomst.topThemas.map(
    (t) => THEMA_BLOKKEN[t],
  );
  ...
  async function verstuur() {
    ...
    setFout(null);
    try {
      // Korte tekst-samenvatting voor in prospect-notitie
      const spiegelTekst = [
        `Totaal-score: ${uitkomst.totaal}/${uitkomst.max} (${uitkomst.pct}%, ${uitkomst.categorieLabel})`,
        ...uitkomst.subScores.map(
          (s) => `${s.label}: ${s.punten}/${s.max}`,
        ),
        "",
  ...
  return (
    <div>
      ...
      {/* 3. Top-thema's uitgebreid uitgelegd */}
      {topThemaBlokken.length > 0 && (
        <div className="mt-8">
          <div className="text-center mb-5">
            <div className="text-orange-600 text-xs font-semibold uppercase tracking-widest">
              Waar voor jou de meeste winst zit
            </div>
      ...
      {/* 5. Algemene basis-leefstijl-tips */}
      <div className="mt-10">
        <div className="text-center mb-5">
          <div className="text-orange-600 text-xs font-semibold uppercase tracking-widest">
            Basis-tips
          ...
          <h3 className="mt-1 text-xl font-bold text-gray-900">
            Wat sowieso werkt, ongeacht je score
          </h3>
        </div>
      ...
      <div className="mt-10">
        <div className="text-center mb-5">
          ...
          <p className="mt-2 text-sm text-gray-600 max-w-md mx-auto">
            Drie niveaus van energie + focus pakketten van Lifeplus.
            Laagdrempelig, zelf in de webshop te bestellen via de
            persoonlijke link van {memberVoornaam}.
          </p>
        </div>
        <PakketKaarten bestellinks={bestellinks} memberVoornaam={memberVoornaam} />
      </div>

      {/* 7. Contact + submit */}
      <section className="mt-8 rounded-2xl border-2 border-orange-100 bg-gradient-to-br from-white to-orange-50/50 p-5 shadow-sm">
      ...
      <button
        ...
        className="mt-6 w-full rounded-full bg-gradient-to-r from-orange-600 to-amber-600 px-6 py-4 text-white text-base font-semibold shadow-lg hover:shadow-xl hover:from-orange-700 hover:to-amber-700 transition-all disabled:from-gray-300 disabled:to-gray-300 disabled:shadow-none"
      >
      ...
      <footer className="mt-8 rounded-2xl border border-gray-200 bg-white/70 backdrop-blur-sm px-4 py-3">
        <p className="text-[11px] text-gray-500 leading-relaxed">
          ...
          raadpleeg altijd je huisarts of professional. We delen
          herkenning en richtingen, geen behandeling. Lifeplus producten
...
function ThemaBlokWeergave({
  ...
  return (
    <div
      className={
        uitgebreid
          ? "mt-5 rounded-2xl border border-orange-200 bg-white p-6 shadow-sm"
          : ""
      }
    >
      {uitgebreid && (
      ...
      <div className="mt-5">
        ...
        <ul className="space-y-2">
          {blok.handvatten.map((h, i) => (
            <li
              key={i}
              className="rounded-xl bg-amber-50/50 border border-amber-100 p-3"
            >
              <div className="font-semibold text-gray-900 text-sm">
                {h.titel}
              </div>
  ...
  const pakketten = [
    {
      ...
      label: "Essential",
      sub: "Instap",
      desc: "Gerichte instap voor energie en focus.",
      badge: "🥉",
    ...
    },
    {
      ...
      desc: "Daily BioBasics + Be Focused + OmeGold voor cognitie.",
      badge: "🥈",
    ...
    },
    {
      ...
      label: "Complete",
      sub: "Volledig pakket",
      desc: "M&P 100 Gold + Be Focused + Cacao Boost voor stemming en scherpte.",
      badge: "🥇",
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {url ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="mt-3 inline-block rounded-full bg-gradient-to-r from-orange-600 to-amber-600 px-4 py-2 text-white text-xs font-semibold shadow-md hover:shadow-lg transition"
                    >
  ...
  return (
    <div className="text-center py-4">
      <div className="relative mx-auto mb-5 h-24 w-24">
        <div className="absolute inset-0 rounded-full bg-gradient-to-br from-emerald-200 to-orange-200 blur-xl opacity-70 animate-pulse" />
        <div className="relative flex h-full w-full items-center justify-center rounded-full bg-gradient-to-br from-emerald-100 to-orange-100 text-5xl shadow-md ring-4 ring-white/60">
          ⚡
        </div>
      </div>
