...
// File: app/core-v9/core-v9-flow.tsx
//
...
// knop die naar /api/core-v9/voltooi-ankerstap POSTet.
...
export function CoreV9Flow({
  ...
  async function voltooiAnkerstap() {
    ...
    try {
      ...
      const res = await fetch("/api/core-v9/voltooi-ankerstap", {
        method: "POST",
        body: form,
      });
    ...
    <main className="mx-auto max-w-2xl px-4 py-6">
      ...
      {huidigeTaak && (
        <>
          ...
          {/* Pre-post / 21-dagen-post-keuze: drie knoppen, één opent
              de pre-post sideflow, één opent de 21-dagen-post sideflow,
              één skipt voor nu (advies blijft hangen). */}
          {huidigeTaak.inlineEmbed === "prepost-keuze" && (
            <div className="mt-4 rounded-lg border-2 border-cm-gold/40 bg-cm-gold/5 p-5 space-y-3">
              <h4 className="text-cm-gold font-semibold text-base">
                📝 Maak je eerste post-keuze
              </h4>
              <p className="text-cm-white/85 text-sm">
                Welke past bij jou nu? Beide brengen mensen in beweging,
                het verschil zit in wat je deelt.
              </p>
              <div className="grid sm:grid-cols-2 gap-3 pt-2">
                ...
                >
                  <p className="text-cm-white font-semibold text-sm mb-1">
                    🌱 Nog geen eigen resultaat
                  </p>
                  <p className="text-cm-white/65 text-xs leading-relaxed">
                    Je gaat een <strong>pre-post</strong> maken: je deelt
                    je voornemen, lokt 'wat ga je doen?' uit.
                  </p>
                </Link>
                <Link
                ...
                >
                  ...
                  <p className="text-cm-white/65 text-xs leading-relaxed">
                    Je gaat een <strong>21-dagen-post</strong> maken: je
                    deelt alle positieve veranderingen, claim-vrij.
                  </p>
                </Link>
              </div>
              <button
                onClick={() => vinkAf(huidigeTaak.id, true)}
                className="mt-2 text-xs text-cm-muted hover:text-cm-white underline italic"
              >
                Voor nu skippen, advies blijft op mijn dashboard staan
              </button>
            </div>
          )}
